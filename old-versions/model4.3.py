...
def generalized_defusionner_graphs(batched_data, batch_size, set_size):
    """Défusionner une liste de DataBatch en utilisant le tensor ptr et les reconstituer en ensembles."""
    all_graphs_by_batch = [[] for _ in range(batch_size)]  # Initialiser une liste pour chaque batch

    for data in batched_data:
        ptr = data.ptr.cpu().numpy()  # Récupérer les séparations pour ce DataBatch
        
        for i in range(batch_size):
            # Isoler les nœuds appartenant au graphe i
            x = data.x[ptr[i]:ptr[i + 1]]
            
            # Isoler les arêtes appartenant au graphe i
            edge_index = data.edge_index[:, (data.edge_index[0] >= ptr[i]) & (data.edge_index[0] < ptr[i + 1])]
            edge_index -= ptr[i]  # Ajuster les indices relatifs
            
            # Isoler les attributs d'arête appartenant au graphe i
            edge_attr = data.edge_attr[(data.edge_index[0] >= ptr[i]) & (data.edge_index[0] < ptr[i + 1])]
            
            # Créer un graphe Data individuel
            graph = Data(x=x, edge_index=edge_index, edge_attr=edge_attr)
            
            # Ajouter le graphe à la liste correspondante dans all_graphs_by_batch
            all_graphs_by_batch[i].append(graph)
    
    return all_graphs_by_batch


def train(model, optimizer, loader, n, batch_size, set_size, leave=False):
    model.train()
    ...
    for i, (batched_data, y_batch) in t:
        ...
        # Défusionner et reconstituer les sets de graphs
        defused_graphs_by_batch = generalized_defusionner_graphs(batched_data, batch_size, set_size)

        batch_outputs = []

        for graphs in defused_graphs_by_batch:
            # Passer les sets de graphs reconstitués au modèle
            output = model(graphs)  # Le modèle reçoit une liste de Data correspondant à un ensemble
