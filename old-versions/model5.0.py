...
data = simu_graph(5, 0.3, 2, 100, 70, 0.10, 0.4, 10)
print(data.T)
...
class DeepSetsModel(nn.Module):
    ...
    def forward(self, data):
        ...
        print('data',data)
        pooled_graphs = global_mean_pool(data.x[:, 0], data.T)
        ...
        out=global_mean_pool(data.x[:, 0], data.batch)  
        print('out',out)    
...
for i, batched_data in t:
    ...
    for j in range(len(batched_data.T)):
        T1.extend([j] * int(batched_data.T[j].sum().item()))
    batched_data.T=T1
    print(len(batched_data.T))
    print(batched_data)
    print(len(batched_data.batch))
    print(batched_data.y)
    batched_data.y=torch.reshape(batched_data.y,(batch_size,2))
    print(batched_data.y)
...
def train(model, optimizer, loader, n, batch_size,leave=False):
    ...
    for i, batched_data in t:
        ...
        print('output',output)
        if output.size(0)!=1:
            raise ValueError(f"exepcted the model to return a single output per databatch but returned {output.size(0)}")
        batch_outputs.append(output.squeeze(0))
        batch_outputs = torch.stack(batch_outputs)

        if batch_outputs.size(0)!=batch_size or batch_outputs.size(1)!=2:
            raise ValueError(f"wrong batch_outputs shape: {batch_outputs.size()}")
